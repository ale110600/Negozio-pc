...
public class ListaPc implements Serializable
{
	 ...
	 * @throws NumberFormatException che si verifica con un inserimento dato non conforme
	 */
	public int eliminaComputer2() throws ListaPcException, NumberFormatException, IOException
	{
		if(elementi==0)
			throw new ListaPcException("lista pc vuota");
		Nodo p;
	 ...
	 * @throws FileException Eccezione che si verifica nel caso in cui il file su cui scrivere è già aperto in lettura.
	 */
	public void esportaCSV2(String nomeFile) throws IOException, ListaPcException , FileException  
	{
		TextFile file=new TextFile(nomeFile,'W');
		String computerCSV;
		Pc computer;
	...
	public int Vendi() throws ListaPcException, NumberFormatException, IOException
	{
		...
		for (int i = 1; i <= getElementi(); i++) 
		{
			...
			if (p.getInfo().getIdentificativo()==identificativo)
			{
				if (p.getInfo().getQuantitàDisponibile()<quantità)
				{
					return 0;
				}
				int x;
				x=p.getInfo().getQuantitàDisponibile()-quantità;
				p.getInfo().setQuantitàDisponibile(x);
				return 1;
			}
		}
		return 2;
	...
	/**
	 * Ordina un array di pc in base alla quantità disponile di ogni pc in ordine crescente
	 * @param array
	 * @return array di metodi ordinato in ordine crescente
	 */
	public static Pc[] OrdinaPcPerQuantitàDisponbile(Pc[] array) throws NumberFormatException, IOException
	{
		...
		for (int i = 0; i < arrayCopia.length-1; i++) 
		{
			...
			{
				if(arrayCopia[i].getQuantitàDisponibile()>=arrayCopia[j].getQuantitàDisponibile())
				{
	...
	*/
	 ...
	 * @throws NumberFormatException che si verifica con un inserimento dato non conforme
	 */
	public String VisualizzaPcQuantitàInferiore() throws NumberFormatException, IOException
	{
